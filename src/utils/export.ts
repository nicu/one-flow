...
const renderComponent = (component: BuilderComponent, indent: number): string => {
  ...
  switch (component.type) {
    case 'text':
      return `${spaces}<div${styleStr}>${component.properties.text || ''}</div>`;
    
    case 'image':
      return `${spaces}<img src="${component.properties.src || ''}" alt="${component.properties.alt || ''}"${styleStr} />`;
    
    case 'button':
      return `${spaces}<button${styleStr}>${component.properties.buttonText || 'Button'}</button>`;
    
    case 'input':
      return `${spaces}<input type="${component.properties.inputType || 'text'}" placeholder="${component.properties.placeholder || ''}"${styleStr} />`;
    
    case 'dropdown': {
      const options = component.properties.options || [];
      return `${spaces}<select${styleStr}>\n${options.map(opt => `${spaces}  <option>${opt}</option>`).join('\n')}\n${spaces}</select>`;
    }
    ...
    case 'grid': {
      ...
      return `${spaces}<div${styleStr}>\n${children.map(c => renderComponent(c, indent + 2)).join('\n')}\n${spaces}</div>`;
    }
...
const buildStyleObject = (properties: ComponentProperties): Record<string, string> => {
  ...
  // Layout properties
  if (properties.flexDirection || properties.gap || properties.justifyContent || properties.alignItems) {
    style.display = 'flex';
    if (properties.flexDirection) style.flexDirection = properties.flexDirection;
    if (properties.gap) style.gap = properties.gap;
    if (properties.justifyContent) style.justifyContent = properties.justifyContent;
    if (properties.alignItems) style.alignItems = properties.alignItems;
  }
  
  if (properties.gridColumns || properties.gridRows) {
    style.display = 'grid';
    if (properties.gridColumns) style.gridTemplateColumns = `repeat(${properties.gridColumns}, 1fr)`;
    if (properties.gridRows) style.gridTemplateRows = `repeat(${properties.gridRows}, 1fr)`;
    if (properties.gap) style.gap = properties.gap;
  }
